...
Because **Gemini Live ignores which Drive folder a sheet lives in**, a document cannot be targeted by folder; instead every document name encodes its targeting: `{store}_{no}_{namespace}_{doctype}_{visibility}_{title}` (e.g. `kim1_1_personal_concepts_public_복리`). This is how "only kim1 knowledge" is selected among all of a person's Google docs, and how one project maps to exactly one knowledge store. Sheets are nonetheless filed under `kim1/{namespace}/{doctype}/` folders for human browsing — organizational only, not a targeting mechanism.
