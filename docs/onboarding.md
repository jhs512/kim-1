...
```
원자료 ──kim-1-learn(증류)──▶ vault/ 마크다운 노드 ──kim-1-sync──▶ Drive 노드시트 ──▶ 폰 Gemini(조회)
                 ▲                    │
                 │              kim-1-ask(조회) · kim-1-check(점검) · kim-1-clean(정제)
          kim-1-calendar(큐 루프)
```
...
| 스킬 | 언제 | 한다 |
|---|---|---|
| **kim-1-learn** | 지식을 넣을 때 | 원자료 → 노드+엣지 증류 (추가 전 조회) |
| **kim-1-ask** | 답을 찾을 때 | vault 그래프 탐색 → 관련 서브그래프 |
| **kim-1-check** | 정기 점검 | 건강도 → 리뷰 큐(고아/저신뢰/끊긴 엣지) |
| **kim-1-clean** | 큐를 비울 때 | 정제: 병합·고아 연결·촘촘화 |
| **kim-1-sync** | 세션 끝 | vault → Drive 노드시트 투영 |
| **kim-1-calendar** | 큐로 넣을 때 | `kim-1-calendar` 캘린더 큐를 루프로 인제스트 |
...
4. 지식 넣기: **kim-1-learn** (또는 `kim-1-calendar`에 이벤트로 큐잉).
5. 끝에 **kim-1-sync**.
