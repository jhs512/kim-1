...
# Layout: kim-1/{namespace}/{document}. Usage: bash scripts/sync.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
declare -A NSF
ensure_ns() { # $1=namespace → echoes subfolder id under $FOLDER (created if absent)
  local ns="$1" id
  [ -z "$FOLDER" ] && { echo ""; return; }
  [ -n "${NSF[$ns]:-}" ] && { echo "${NSF[$ns]}"; return; }
  local q="name = '$ns' and mimeType='application/vnd.google-apps.folder' and '$FOLDER' in parents and trashed=false"
  ...
  [ -z "$id" ] && id=$("$GWS" drive files create --json "{\"name\":\"$ns\",\"mimeType\":\"application/vnd.google-apps.folder\",\"parents\":[\"$FOLDER\"]}" --format json 2>/dev/null | jget '(JSON.parse(d)).id||""')
  NSF[$ns]="$id"; echo "$id"
}
...
# 2) I/O: project each node into kim-1/{namespace}/, persist manifest
: > "$SYNC_DIR/manifest.tsv.new"
while IFS=$'\t' read -r no name id visibility namespace base; do
  [ -z "${no:-}" ] && continue
  nsfolder=$(ensure_ns "$namespace")
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$no" "$name" "$id" "$visibility" "$namespace" "$sid" >> "$SYNC_DIR/manifest.tsv.new"
done < "$OUT/plan.tsv"
