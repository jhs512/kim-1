...
# kim-1-clean — 정제(그래프 건강 회복)

[[kim-1-check]]가 뽑은 리뷰 큐를 받아 vault 그래프를 **결정적으로 고친다**. 마크다운을
...
3. **끊긴 엣지 수리** — `target`이 없는 엣지: 대상 노드를 [[kim-1-learn]]로 만들거나, 엣지를 올바른 id로 고친다.
