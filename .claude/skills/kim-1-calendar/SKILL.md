---
name: kim-1-calendar
description: Google 캘린더 'kim-1-calendar'를 입력 큐로 삼아, 미처리 이벤트를 하나씩 kim-1 노드로 증류·투영하고 완료 표시하는 큐 루프 스킬. 트리거 - "캘린더 큐", "큐 돌려", "캘린더에서 받아와", "kim-1-calendar 처리", "인제스트 루프".
---

# kim-1-calendar — 캘린더 큐 인제스트 루프

`kim-1-calendar`란 Google 캘린더를 **입력 큐**로 쓴다. 각 **이벤트 = 큐 아이템**(제목 = 주제,
설명 = 원자료 텍스트 또는 URL). 미처리 이벤트를 하나씩 꺼내 [[kim-1-learn]]로 증류하고
[[kim-1-sync]]로 Drive에 반영한 뒤 **비파괴 완료 표시**(제목 앞 `✅`)한다. 삭제하지 않는다.
