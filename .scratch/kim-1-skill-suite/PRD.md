...
추가로 **`kim-1-calendar` Google 캘린더를 입력 큐로 삼아 이벤트를 루프로 증류·반영**한다.
...
| 스킬 | 역할 (eb 원본) |
|---|---|
| **kim-1-learn** | 증류: 원자료 → infinite-brain 노드(+엣지) 마크다운. 추가 전 조회로 중복/연결 확인 (eb-learn) |
| **kim-1-ask** | 조회: vault 그래프 탐색 → 관련 서브그래프 (eb-ask) |
| **kim-1-check** | 건강도 점검 → 리뷰 큐(저신뢰/고아/끊긴 엣지/빈 필드) (eb-check) |
| **kim-1-clean** | 정제: 중복 병합·고아 연결·촘촘화 (eb-clean) |
| **kim-1-sync** | 투영: `scripts/sync.sh` 래핑(knowledge-work 끝 → Drive) (신규) |
| **kim-1-calendar** | `kim-1-calendar` 캘린더 큐를 루프로 인제스트 (신규·요청) |
...
## kim-1-calendar 큐 (핵심 신규 기능)

- Google 캘린더 **`kim-1-calendar`** 를 입력 큐로 사용(gws로 접근).
- **이벤트 = 큐 아이템**: 제목/설명에 원자료 텍스트 또는 URL.
- 루프: 미처리 이벤트 순회 → `kim-1-learn` 증류(노드 생성) → `kim-1-sync` Drive 반영 →
...
- `kim-1-calendar` 캘린더 생성 + 이벤트 1개를 루프가 증류→노드→시트→완료표시까지 실증.

## Comments

### 2026-07-01 — 구현 완료
- 스킬 6종(전부 한글): `kim-1-learn/ask/check/clean/sync/calendar` + `docs/onboarding.md`
- 쿼리 엔진: `scripts/lib/graph.mjs`(search/neighbors/health, TDD) + `scripts/kim.mjs` CLI
- 캘린더 큐: `scripts/kim-cal.sh`(ensure/list/done/add) — `kim-1-calendar` 캘린더 생성됨(id는 `.sync/calendar-id`)
